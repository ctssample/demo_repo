--example: hive -f create_rio_prag_incoming_gprs_table.hql --hivevar env=dev --hivevar nameNode=hdfs://nameservice1

CREATE EXTERNAL TABLE IF NOT EXISTS ${env}_swe_base.t_rio_prag_incoming_gprs( 
...
`BNET` VARCHAR(6)  COMMENT 'NATTILLHORIGHET',
...
`ATJANST_2` VARCHAR(5)  COMMENT 'TJANST',
`NTJANST` VARCHAR(5)  COMMENT 'NAT_REALTERAD_TJANST',
...
`TINFOFLG` VARCHAR(1)  COMMENT 'TILLAGGS_INFO-FLAGG',
`PT_2` VARCHAR(2),
`UDATA` BIGINT,
`NETUSE` VARCHAR(1),
`LANDSKOD` VARCHAR(4),
...
COMMENT 'Base table for RiO prag incoming GPRS feed' 
PARTITIONED BY(schdate_year string, schdate_month string, schdate_day string , schtime_hour string)
STORED AS AVRO
 LOCATION 
 '${nameNode}/data/${env}/swe/base/rio/rio_prag_incoming_gprs' ;
 
 MSCK REPAIR TABLE ${env}_swe_base.t_rio_prag_incoming_gprs;