--example: hive -f create_rio_prag_incoming_gsm_table.hql --hivevar env=dev --hivevar nameNode=hdfs://nameservice1

CREATE EXTERNAL TABLE IF NOT EXISTS ${env}_swe_base.t_rio_prag_incoming_gsm ( 
...
COMMENT 'Base table for RiO prag incoming GSM feed' 
PARTITIONED BY(schdate_year string, schdate_month string, schdate_day string , schtime_hour string)
stored as AVRO  
 LOCATION 
   '${nameNode}/data/${env}/swe/base/rio/rio_prag_incoming_gsm' ;
   
   MSCK REPAIR TABLE ${env}_swe_base.t_rio_prag_incoming_gsm;
